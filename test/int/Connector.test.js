...
describe('Connector INT', () => {
  ...
  describe('Source connects and streams', () => {
    ...
    it('should be able to run BigQuery source config', () => {
      ...
      return runSourceConnector(sourceProperties, [], onError).then(
        (_config) => {
          config = _config;
  ...
  describe('Sink connects, inserts, retries on error', () => {
    ...
    before('Produce 4 messages', () => {
      ...
      return producer.connect().then((_) => {
        return Promise.all([
          producer.buffer(topic, '10', {
            payload: { id: 10, name: 'test10', info: null },
            type: 'publish',
          }),
          producer.buffer(topic, '11', {
            payload: { id: 11, name: 'test11', info: null },
            type: 'publish',
          }),
          producer.buffer(topic, '12', {
            payload: { id: 12, name: 'test12', info: null },
            type: 'publish',
          }),
          producer.buffer(topic, '13', {
            payload: { id: 13, name: 'test13', info: null },
            type: 'publish',
          }),
        ]);
      });
    ...
    it('should be able to run the BigQuery sink config', () => {
      ...
      return runSinkConnector(sinkPropertiesRetry, [converter]).then(
        (_config) => {
          config = _config;
          config.on('model-upsert', (id) => console.log('upsert: ' + id));
          config.on('model-delete', (id) => console.log('delete: ' + id));
  ...
  describe('Sink swallows table exists errors, if multiple instances try to create it concurrently', () => {
    ...
    it('should be able to produce a message', () => {
      const producer = new Producer(
        sinkProperties.kafka,
        sinkProperties.topic,
        1,
      );
      return producer
        .connect()
        .then(() =>
          producer.buffer(sinkProperties.topic, '3', {
            payload: { id: 9, name: 'dummy', info: 'for swallow test' },
            type: 'publish',
          }),
        );
    });
    ...
    it('should be able to run the BigQuery sink config', () => {
      const converter = ConverterFactory.createSinkSchemaConverter(
        ...
        (messageValue, callback) => callback(null, messageValue.payload),
      );

      const onError = (_error) => {
        error = _error;
      };
      return runSinkConnector(sinkProperties, [converter], onError).then(
        (_config) => {
          config = _config;
          return true;
        },
  ...
  describe('Converter Factory', () => {
    ...
    it('should be able to create a custom converter', (done) => {
      ...
      converter = ConverterFactory.createSinkSchemaConverter(
        bigQueryTableDescription,
        etlFunc,
      );
      ...
      converter.toConnectData(
        Object.assign({}, aFakeKafkaMessage),
        (error, message) => {
          assert.ifError(error);
    ...
    it('should be able to produce a few messages', () => {
      ...
      return producer.connect().then((_) => {
        return Promise.all([
          producer.buffer(topic, '3', {
            payload: { id: 3, name: 'test1', info: null },
            type: 'publish',
          }),
          producer.buffer(topic, '4', {
            payload: { id: 4, name: 'test2', info: null },
            type: 'publish',
          }),
    ...
    it('shoud be able to sink message through custom converter', () => {
      ...
      return runSinkConnector(customProperties, [converter], onError).then(
        (_config) => {
          config = _config;
          return true;
        },
