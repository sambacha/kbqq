...
class FakeTable {
  constructor(id, datasetId, projectId) {
    ...
    this._description = JSON.parse(
      JSON.stringify(FakeTable.nextValues.description),
    );
  ...
  insert(rows, options, callback) {
    ...
    return callback(null, {
      raw: options.raw,
      rowCount: FakeTable.lastInsertedRows.length,
    });
  }
  ...
  getRows(options, callback) {
    ...
    const resultPageEnd = Math.min(
      resultPageStart + (options.maxResults || this._rows.length),
      this._rows.length,
    );
