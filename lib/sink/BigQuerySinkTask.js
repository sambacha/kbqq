...
class BigQuerySinkTask extends SinkTask {
  ...
  runBatch() {
    const rows = this.buffer.splice(
      0,
      Math.min(this.batchSize, this.buffer.length),
    );

    return new Promise((resolve, reject) =>
      this.table.insert(rows, { raw: true }, (error, apiResponse) => {
        ...
        rows.forEach((row) =>
          this.parentConfig.emit('model-upsert', row.insertId),
        );
  ...
  beforeFirstPut(record) {
    ...
    return new Promise((resolve, reject) =>
      this.table.create(tableDescription, (error, table, apiResponse) => {
        if (error) {
          ...
          if (
            error.code === 409 &&
            error.message.startsWith('Already Exists:')
          ) {
            return resolve();
          }
  ...
  _ensureDatasetExists(callback) {
    this.dataset.exists((error, exists) => {
      if (error) {
        return callback(
          new Error(
            `Failed to check if dataset exists: ${JSON.stringify(error)}`,
          ),
        );
      }
  ...
  _checkTableExists(callback) {
    this.table.exists((error, exists) => {
      if (error) {
        return callback(
          new Error(
            `Failed to check if table exists: ${JSON.stringify(error)}`,
          ),
        );
      }
